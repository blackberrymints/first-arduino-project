...
class Display {
  public:
    ...
    // Redraws ONLY the small sprite area (not the whole screen) - call this
    // every loop() so sleep/pet animations keep cycling frames smoothly,
    // without the full-screen flicker a complete redraw would cause.
    void updateSprite(PetMood mood);

  private:
    Adafruit_ST7735 _tft{5, 2, 4}; // CS, DC, RST
